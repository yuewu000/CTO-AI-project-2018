...
#    f = open("./input/" + basename + str(index) + ".csv", 'rb').read()
    ...
    e = pd.read_csv("/notebooks/input/" + stockname + No + ".csv")
...
#change current dir to "/notebooks"
os.chdir('/notebooks')
cwd = os.getcwd()
print ('The current dir is: {};\n\nAnd the files in the dir are: {}'.format(cwd, os.listdir(cwd)))
print ('Input files in the input dir are: {}'.format(os.listdir(os.path.join(cwd, 'input'))))
...
prep_rdd = sc.parallelize(zip_2d, 38)
...
print "Optimization Done. Total {} epochs finished in {} S".format(epochs, time.time()-start_time)
